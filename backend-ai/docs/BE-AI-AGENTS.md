...
## 0. Context About The Project
`backend-ai`, as the name shows, is a directory of backend and AI logic for a prototipe called PIVO. See `README.md` for more context. More details are presented in `docs/*.html`.
...
**The Dev Log (`BE-AI-DEVLOG.md`)**
At the completion of a task, you must check if `BE-AI-DEVLOG.md` exists. If so, propose an append summarizing:
...
**Goal:** If a new developer (or a new AI session) joins tomorrow, they should be able to read `BE-AI-DEVLOG.md` and understand the state of the project immediately.

**Operational Rule**
- After every interaction that includes a code change, you must append an entry to `BE-AI-DEVLOG.md` before finishing. Do not just suggest it. If you truly cannot write to the file (permissions/conflicts), provide the exact snippet the next person should paste. This is mandatory and should be treated as a checklist item for every task.